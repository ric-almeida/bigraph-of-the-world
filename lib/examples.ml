  ...
  let wgb =
    Big.close
      ...
      (Big.ppar
         (Big.atom
            (Link.parse_face [ "building_id" ])
            Ctrl.{ s = "ID"; p = [ S "r8388-William Gates Building" ]; i = 1 })
         (Big.par (Big.nest building Big.one) site))
  in
  let cavendish_wgb =
    Big.comp
      (Big.close
            ...
            (Big.par
               (Big.atom
                  ...
                  Ctrl.
                    {
                      s = "ID";
                      p = [ S "w164365618-Cavendish Laboratory" ];
                      i = 1;
                    })
               site)
            (Big.par (Big.nest building Big.one) site)))
      ...
      (Big.close
         ...
         (Big.ppar
            (Big.par
               (Big.atom
                  (Link.parse_face [ "street_id" ])
                  Ctrl.{ s = "ID"; p = [ S "J J Thompson Avenue" ]; i = 1 })
               site)
            (Big.par street site)))
      (Big.ppar cavendish_wgb site)
  in
  let cambridge =
    Big.comp
      (Big.close
         ...
         (Big.ppar
            (Big.par
               (Big.atom
                  (Link.parse_face [ "boundary_id" ])
                  Ctrl.{ s = "ID"; p = [ S "8-295355-Cambridge" ]; i = 1 })
               site)
            (Big.par boundary site)))
      (Big.ppar jjthompson site)
  in
  let cambridgeshire =
    Big.comp
      (Big.close
         ...
         (Big.ppar
            (Big.par
               (Big.atom
                  (Link.parse_face [ "boundary_id" ])
                  Ctrl.{ s = "ID"; p = [ S "6-180837-Cambridgeshire" ]; i = 1 })
               site)
            boundary))
      cambridge
  in
