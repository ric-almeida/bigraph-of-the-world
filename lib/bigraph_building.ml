...
let build_place_graph (root_level : string) (root_id : string) (root_name : string) (id_in_parameter:bool)= 
    let root_string = root_level^"-"^root_id^"-"^root_name in
    let _ = Overpass.query_all_children root_level root_id root_name in
            ...
            let rec helper boundary_string id_seen comp_list = 
                ...
                let (id_seen, boundary_children_bigraphs) = 
                    ...
                    List.fold children_boundaries 
                        ~init:(id_seen, [(Big.ppar site Big.one)]) (* site for id and one to close sibling boundaries *)
                        ~f:(fun (id_seen, boundary_children_bigraphs) child_boundary_string -> 
                            helper child_boundary_string id_seen boundary_children_bigraphs) in
                let (id_seen, buildings_with_no_street, street_name_to_way_ids_and_buildings, outer_names) = Hierarchy.idseen_buildingswithnostreet_streetnametowayidsandbuildings_outernames boundary_string id_seen in
                let (id_seen, way_id_to_junctions) = Hierarchy.way_id_to_junctions boundary_string id_seen outer_names in
                let boundary_children_bigraphs = 
                    Set.fold buildings_with_no_street ~init:boundary_children_bigraphs ~f:(fun boundary_children_bigraphs building_name->
                        (Big.close
                            ...
                            (Big.ppar
                                (Big.par 
                                    ...
                                    else Big.atom (Link.parse_face ["id"]) Ctrl.{ s = "ID"; p = [S building_name]; i = 1 })
                                )
                                (Big.par
                                    site (* sibiling building/street/boundary*)
                                    (Big.atom (Link.parse_face (if id_in_parameter then [] else ["id"])) Ctrl.{ s = "Building"; p = if id_in_parameter then [S building_name] else []; i = if id_in_parameter then 0 else 1 })
                                )
                            )
                ...
                let bigraph_and_siblings =
                            ...
                            (
                                (Big.ppar
                                    (Big.par
                                        site (* sibiling id*)
                                        (if id_in_parameter then Big.id_eps
                                        else Big.atom (Link.parse_face ["id"]) Ctrl.{ s = "ID"; p = [S boundary_name]; i = 1 })
                                    )
                                    (Big.par
                                        site (* sibiling building/street/boundary*)
                                        (Big.ion (Link.parse_face (if id_in_parameter then [] else ["id"])) Ctrl.{ s = "Boundary"; p = if id_in_parameter then [S boundary_name] else []; i = if id_in_parameter then 0 else 1 }) (* site for children*)
                                    )
                                )::
                                (Big.placing [[0];[2];[1]] 3 Link.Face.empty):: (* children only have 2 regions, parent has 3 sites. add site to right then move to middle*)
                                        ...
                                        (Big.close
                                            ...
                                            (Big.ppar
                                                (Big.par
                                                    site (* sibiling id*)
                                                    (if id_in_parameter then Big.id_eps
                                                    else Big.atom (Link.parse_face ["id"]) Ctrl.{ s = "ID"; p = [S street_name]; i = 1 })
                                                )
                                                (Big.par
                                                    site (* sibiling street/boundary*)
                                                    (Big.ion (Link.parse_face (if id_in_parameter then [] else ["id"])) Ctrl.{ s = "Street"; p = if id_in_parameter then [S street_name] else []; i = if id_in_parameter then 0 else 1 }) (*site for children*)
                                                )
                                            ) 
                                        )::(* nest*)
                                        (let junction_bigs = 
                                            List.fold junctions 
                                                ~init:((Big.ppar site (Big.ppar site Big.one))::boundary_children_bigraphs) (* close open-ended buildings sibling*)
                    ...
                    in
                    (Big.close
                        (Link.Face.diff (Big.face_of_inter (Big.outer boundary_graph)) (Link.parse_face (Set.to_list outer_names)))
                        boundary_graph
                    )::comp_list
                    in
                let _ = report_progress 1 in
                (id_seen, bigraph_and_siblings) in
            let (_, bigraph_list) = helper root_string String.Set.empty [Big.ppar Big.one Big.one] in
...
let add_agent_to_building_react ~bigraph ~agent_id ~building_name= 
    ...
    let parent_id = Big.atom (Link.parse_face ["id"]) Ctrl.{ s = "ID"; p = [S building_name]; i = 1 } in
    let lhs = 
        Big.close
            (Link.parse_face ["id"])
    ...
    let react_add_agent = BRS.parse_react_unsafe ~name:("Add agent "^agent_id^" to building "^building_name)  ~lhs:lhs ~rhs:rhs () None in
    match BRS.step bigraph [react_add_agent] with
    | ((x,_,_)::_,1) ->  x
    | (_,n) -> raise (Not_found_s (Sexplib0.Sexp.message ("Building name \""^building_name^"\" not unique. Number of matches: "^(string_of_int n)) []))
...
let street2 = Big.ion (Link.parse_face ["street_id2"]) Ctrl.{ s = "Street"; p = []; i = 1 }
let junction = Big.atom (Link.parse_face ["junction_id"]) Ctrl.{ s = "Junction"; p = []; i = 1 }
...
let react_move_out_of_boundary = 
    ...
    BRS.parse_react_unsafe ~name:"Move out of boundary"  ~lhs:lhs ~rhs:rhs () None
...
let react_move_into_boundary = 
    ...
    let rhs = 
        Big.nest 
            boundary
            (Big.par site agent) in
    BRS.parse_react_unsafe ~name:"Move into boundary"  ~lhs:lhs ~rhs:rhs () None
...
let react_move_out_of_street =
    ...
    BRS.parse_react_unsafe ~name:"Move out of street"  ~lhs:lhs ~rhs:rhs () None
...
let react_move_into_street =
    ...
    let rhs = 
        Big.nest 
            street
            (Big.par site agent) in
    BRS.parse_react_unsafe ~name:"Move into street"  ~lhs:lhs ~rhs:rhs () None
...
let react_move_out_of_building =
    ...
    BRS.parse_react_unsafe ~name:"Move out of building"  ~lhs:lhs ~rhs:rhs () None
...
let react_move_into_building =
    ...
    let rhs = 
        Big.nest 
            building
            (Big.par site agent) in
    BRS.parse_react_unsafe ~name:"Move into building"  ~lhs:lhs ~rhs:rhs () None
    ...
    let lhs = 
        Big.close
            ...
            (Big.ppar 
                (Big.nest 
                    street
                    (Big.par site (Big.par junction agent)))
                (Big.nest 
                    street2
                    (Big.par junction site))
             ) in
    let rhs = 
        Big.close
            (Link.parse_face ["junction_id"])
            (Big.ppar 
                (Big.nest 
                    street
                    (Big.par site junction))
                (Big.nest 
                    street2
                    (Big.par agent (Big.par junction site)))
             ) in
    BRS.parse_react_unsafe ~name:"Move to linked street"  ~lhs:lhs ~rhs:rhs () None
...
let react_rules = 
    [
        react_move_out_of_building; react_move_into_building; 
        react_move_out_of_street; react_move_into_street; 
...
let agent_in_building = 
    Big.nest building (Big.par site agent)