...
(** downloads osm file of all admin boundaries, buildings and streets in area defined by root_id, as well as outer names respresnting intersections between streets inside and outside area. https://overpass-turbo.eu/s/233f *)
...
let query_all_children (root_level : string) (root_id : string) (root_name :string) =
    let _ = print_endline "Querying Overpass API for boundaries not yet saved. Will take a while..." in
    ...
    let bar total =
        ...
        list [ const "Querying for child boundaries"; elapsed (); bar total; count_to total ] in
